...
sampling.df$All_Minnows_Images_sp[9] <- paste0(nrow(presence.meta.scale),
                                               " (",
                                               length(unique(presence.meta.scale$scientificName.x)),
                                               ")")
...
sampling.df$Burress_et_al._2017_Overlap_Images_sp[9] <- paste0(nrow(presence.meta.scale[presence.meta.scale$scientificName.x %in% b.sp,]),
                                                               " (",
                                                               length(unique(presence.meta.scale$scientificName.x[presence.meta.scale$scientificName.x %in% b.sp])),
                                                               ")")
...
sampling.df$All_Minnows_Images_sp[10] <- paste0(nrow(df.fin.per.sample),
                                               " (",
                                               length(unique(df.fin.per.sample$scientificName.x)),
                                               ")")
...
sampling.df$Burress_et_al._2017_Overlap_Images_sp[10] <- paste0(nrow(df.fin.per.sample[df.fin.per.sample$scientificName.x %in% b.sp,]),
                                                                " (",
                                                                length(unique(df.fin.per.sample$scientificName.x[df.fin.per.sample$scientificName.x %in% b.sp])),
                                                                ")")
...
# Remove NA columns that cause `NA/NaN/Inf in foreign function call (arg 10)`  error
stats.sp.trim <- stats.sp.trim[ , colSums(is.na(stats.sp.trim))==0]
...
colnames(b.sampling) <- c("scientificName", "Sample_Size")
...
colnames(sampling.95.3) <- c("scientificName", "Sample_Size")
