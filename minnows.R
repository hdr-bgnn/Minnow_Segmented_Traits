...
####LOAD DATA----
ml.fish.data <- read.csv("fish.meta.qual.tax.csv", header = TRUE)
image.data <- read.csv("Image_Metadata_v1.1_20220315_131256.csv", header = TRUE)

minnows <- image.data[image.data$family == "Cyprinidae",] %>% drop_na()
minnow.noDupe <- minnows[!duplicated(minnows$catalog_id),] %>% drop_na()
